...
function ScreenCard({ app, index, onScreened }) {
  ...
  const runScreen = async () => {
    ...
    const cv = app.cv_text || app.original_cv || ''
    ...
    try {
      ...
      setResult(parsed)
      await onScreened(app.id, parsed.score, parsed.summary)
    } catch (e) {
      setErr(e.message)
    }
    setLoading(false)
    ...
    <div style={s.card}>
      ...
      <div style={s.cardTop}>
        <div>
          ...
          <div style={s.roleText}>{app.applicant_role || app.jobs?.title || 'Unknown Role'}</div>
        </div>
        <button style={s.screenBtn(loading)} onClick={runScreen} disabled={loading}>
          {loading && <Spinner />}
      ...
      {app.cv_text && (
        <div style={s.cvPreview}>
          {app.cv_text.slice(0, 200)}{app.cv_text.length > 200 ? '…' : ''}
        </div>
      )}
...
export default function BlindScreener() {
  const { applications, loading, error, updateBlindScore } = useApplications()
